...
void _showAutoStopSettingsModal(BuildContext context, controller) {
  ...
  final timeController = TextEditingController(text: currentData?.autostopTime?.toString() ?? '');
  final unitController = TextEditingController(text: currentData?.autostopUnit?.toString() ?? '');
  final priceController = TextEditingController(text: currentData?.autostopPrice?.toString() ?? '');
          ...
          Obx(() => Column(
            children: [
              Row(
                ...
                children: [
                  Text('Stop After Time', style: Theme.of(context).textTheme.bodyLarge),
                  Switch(
          ...
          Obx(() => Column(
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Stop After Units', style: Theme.of(context).textTheme.bodyLarge),
                  Switch(
          ...
          Obx(() => Column(
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Stop After Price', style: Theme.of(context).textTheme.bodyLarge),
                  Switch(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    try {
                      ...
                      if (isPriceEnabled.value && priceController.text.isEmpty) {
                        throw 'Please enter price value or disable price setting';
                      }
                      if (isTimeEnabled.value && int.tryParse(timeController.text) == 0) {
                        throw 'Time value must be greater than 0';
                      }
                      if (isUnitEnabled.value && int.tryParse(unitController.text) == 0) {
                        throw 'Unit value must be greater than 0';
                      }
                      if (isPriceEnabled.value && double.tryParse(priceController.text) == 0) {
                        throw 'Price value must be greater than 0';
                      }
                      final updateData = {
                        'autostop_time': timeController.text.isNotEmpty ? int.tryParse(timeController.text) : null,
                        'autostop_time_is_checked': isTimeEnabled.value,
                        'autostop_unit': unitController.text.isNotEmpty ? int.tryParse(unitController.text) : null,
                        'autostop_unit_is_checked': isUnitEnabled.value,
                        'autostop_price': priceController.text.isNotEmpty ? double.tryParse(priceController.text) : null,
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  ...
                                  try {
                                    ...
                                    // Error handling in endChargingSession
                                  }
                                },
                              ),
                              Row(
                                children: [
                                  Obx(() {
                                    final isCharging = controller.chargingData.value?.chargerStatus == 'Charging';
                                    return Stack(
                                      ...
                                      children: [
                                        IconButton(
                                          icon: Icon(
                                            Icons.settings,
                                            color: isCharging ? Colors.indigo.withOpacity(0.5) : Colors.indigo,
                                          ),
                                          onPressed: isCharging
                                              ? null
                                              : () => _showAutoStopSettingsModal(context, controller),
                                        ),
                                        if (isCharging)
                                          Positioned(
                                            left: 8,
                                  ...
                                  GestureDetector(
                                    onTap: () => Get.to(() => ContactUs(), transition: Transition.rightToLeft),
                                    child: Container(
                        ...
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Container(width: 2, height: screenHeight * 0.04, color: theme.dividerColor),
                                  SizedBox(width: screenWidth * 0.01),
                                  Container(width: 2, height: screenHeight * 0.04, color: theme.dividerColor),
                                ],
                              ),
                              Row(
                                children: [
                                  ...
                                  SizedBox(width: screenWidth * 0.02),
                                  Text(connectorType == 1 ? 'Socket' : 'Gun', style: textStyle),
                                ],
                              ),
                            ],
                          ),
                                    ...
                                    child: SliderButton(
                                      action: () async {
                                        ...
                                        try {
                                          ...
                                          isStopButtonDisabled.value = true; // Disable on success
                                          return true;
                                        } catch (e) {
                          ...
                          child: Obx(() {
                            ...
                            if (chargerStatus == 'Preparing') {
                              return SliderButton(
                                ...
                                icon: Icon(Icons.bolt, color: Colors.green, size: 30),
        ...
        children: [
          AnimatedBuilder(
            ...
            builder: (context, child) {
              return Transform(
                alignment: Alignment.center,
                transform: Matrix4.rotationY(controller.rotationController.value * 1 * pi),
                child: Image.asset(
