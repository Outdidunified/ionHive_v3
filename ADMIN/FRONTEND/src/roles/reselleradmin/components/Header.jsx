...
const Header = ({ handleLogout, userInfo }) => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/reselleradmin/FetchPaymentNotification', {
      ...
      isFetching.current = false; // Reset fetching flag when done
    }
  };
  ...
  const markAsRead = async (notificationId, event) => {
    ...
    event.stopPropagation(); // Prevent click from closing the notification box
    ...
    try {
      ...
      const response = await axiosInstance.post('/reselleradmin/MarkNotificationRead', {
        _id: notificationId,
  ...
  return (
    <nav className="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row" style={{ backgroundColor: 'white' }}>
      ...
      <Link className="navbar-brand brand-logo mr-5" to="/reselleradmin/Dashboard">
          <img src="../../images/dashboard/EV-RESELLER-ADMIN.png" className="mr-2" alt="logo" style={{ paddingLeft: 10 }} />
        </Link>
        <Link className="navbar-brand brand-logo-mini" to="/reselleradmin/Dashboard">
